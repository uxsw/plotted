...
See Shapes. A framed, matted, captioned photograph with a mono specimen number — the device that makes an editorial page read as a catalogue. Reserved for marketing / editorial surfaces; in-app imagery is otherwise plainer (bottom-rounded, uncaptioned). The one in-app exception is the scheme-entry photograph below.

### Scheme entry (`c-scheme-entry`)

The `/plant-scheme` front door — a Persuade surface inside the app, the one place the catalogue turns its cover outward. Its job is to explain a tool the gardener hasn't used yet, so it **shows the output** rather than describing it: the explanation and the proof are one annotated example. Sequence: promise → annotated example → the two doors. All on the ~800px measure.

- **Head** (`.c-scheme-entry__head`): a `.canon .kirk` title, a one-sentence `.primer` lead, and a ghost "Choose how to start ↓" jump link (to `#scheme-start`) so returning gardeners skip the explanation. Beside it on desktop (stacked under the lead below `40rem`), a `15rem` **image plate** of `garden-hero-2.jpg` — paper mat, hairline frame, `4/5` (`3/2` on a phone), caption below. The photograph supports the promise; it carries no copy.
- **Annotated example** (`.c-scheme-example`, `SchemeExample.tsx`): a season-washed letterhead (`Example scheme` · `Sunny border, free-draining soil`) over the journey's three steps as an `<ol>` — each a numbered `--_accent` roundel stamp + `.long-primer .kirk` heading + `n-deep-grey` note in a `16rem` column, beside the real part that step produces: (1) starting-plant pills, (2) a real `ChatMessage` question with its quick replies, the chosen one in the cyan Chosen-Is-Filled edge, and "Skip", (3) an open hairline sheet with `BorderElevation`, `FloweringYear` and two `PlantCard`s whose "why this fits" line is genuinely computed by `buildMatchNote` (one card below `32rem`). A 1px accent path joins the stamps on desktop. Its accent is **early summer**, a season neither door uses, so the example never reads as a path. Content comes from `mockData.ts`, so the example tracks the journey. Deliberately non-interactive — a clickable fake would promise a demo that isn't there.
- **Authored moment:** the sheet starts as the elevation's dashed ghosts; the first time it is 40% in view (IntersectionObserver) the elevation remounts with the plants, running its own staggered draw-in, and the months fill. Ghost state is `aria-hidden` so its "nothing yet" census isn't read. Reduced motion: final state.
- **Doors** (`#scheme-start`, `.c-scheme-plate`, grid `auto-fit minmax(19rem, 1fr)`): white raised cards, `18rem` min height, a `1px` sand-line seat shadow at rest, an `8rem` season colour field (see the Semantic Field exception) with the engraving `__mark` and a hue-neutral tonal deepen. Body: `.pica .kirk` title, season-tinted `.brevier` note, mono `START` + arrow icon pinned to the base. **From your garden** is personal: the note carries the real count ("the 14 plants already in your garden", singular for one), and up to four overlapping `3rem` roundels of the gardener's own photos (sprout where a plant has none) plus `+N` sit in the field. Zero plants: the door becomes a non-button **ghost** (`.is-unavailable` — transparent, dashed sand-line edge, neutral field) saying so and linking to `/plants/new`. A failed read falls back to generic copy, never to "empty".
- **Hover** (incidental, default curve — the example owns the authored moment): field deepens, engraving firms (`0.4 → 0.52`), plate lifts `2px` with the card-hover shadow, arrow steps `0.25rem`. **Focus:** the standard button ring over the lift.
...
- **Do** treat the `/plant-scheme` entry as the app's one sanctioned Persuade surface — an image-plate garden photograph, an annotated example of the output, and large `--sem-flowering-*` colour fields belong *there*; on an Operate screen they are drift.
...
- **Don't** copy the scheme-entry photo plate, annotated example, or its coloured choice fields onto another app route — it is a scoped front-door treatment, not a new app-wide pattern (see Scheme entry).
