...
export default async function PlantsPage() {
  ...
  const [{ data: plants, error }, { data: pwaFlags }] = await Promise.all([
    supabase
      .from("plants")
      .select("*")
      .eq("status", "active")
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <PwaInstallTriggerCard
        initialDismissCount={pwaFlags?.pwa_prompt_dismiss_count ?? 0}
        initialInstalledAt={pwaFlags?.pwa_installed_at ?? null}
