...
- **Scheme forking.** Real data-modelling complexity (shared vs. snapshot history, multi-scheme UI) with no validated user demand yet. Revisit once usage shows people want variations.
...
- As a user who isn't happy with a generated scheme, I want to reopen and continue refining it, so that I don't have to start over.
...
- Once a scheme is generated, Plotted performs a best-effort Wikimedia image lookup per plant for illustration purposes only.
- Missing images do not block or alter the scheme in any way.
- *Acceptance criteria:*
  - Given a scheme has been generated, then each plant is checked against Wikimedia for an image; plants without a match display without an image, with no error state.

**Shopping list integration**
- Each recommended plant has an "add to shopping list" action, replicating current shopping list behaviour exactly.
- Downstream shopping list behaviour (mark as purchased → full plant lookup / add to garden) is unchanged.
- *Acceptance criteria:*
  - Given a generated scheme, when the user adds a recommended plant to their shopping list, then it behaves identically to adding a plant to the shopping list from the current scheme feature.

**Reopen and continue**
- A generated scheme can be reopened and its chat continued; regenerating overwrites the existing scheme in place (no version history in phase 1).
- *Acceptance criteria:*
  - Given a previously generated scheme, when the user reopens it, then the prior chat context is available and further messages can refine the existing scheme.
  - Given a reopened scheme is regenerated, then it replaces the previous version of that scheme rather than creating a new one.
...
**Leading indicators**
- Completion rate: % of started chats that reach a generated scheme (any exit path — full flow, skipped, or quick-answered).
...
- Shopping list conversion: % of generated schemes with at least one plant added to the shopping list.
